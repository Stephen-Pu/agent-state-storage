"""High-level Python wrapper around the Core ABI.

This adapter is the *thin* Pythonic surface that engine integrations
(vLLM's ``KVConnectorBase``, SGLang's L2 backend, ad-hoc demos) build upon.
The ``KVCacheConnector`` class below is deliberately engine-agnostic — the
vLLM-specific wiring lives in a separate module (TODO(stephen)).
