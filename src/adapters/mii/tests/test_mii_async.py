"""AsyncLoadDriver + AsyncMIIKVCache tests (fake connector for
...
from kvcache_mii import AsyncLoadDriver
...
def test_async_schedule_load_collect_round_trip():
    from kvcache_mii import AsyncMIIKVCache
    tokens = list(range(2 * BLOCK))
    payload = bytes(((i * 23) & 0xFF
                      for i in range(len(tokens) * BYTES_PER_TOKEN)))
    with AsyncMIIKVCache(tenant_id="mii-async", model_id="mii-async-demo",
                         bytes_per_token=BYTES_PER_TOKEN, workers=2) as kv:
        kv.put(tokens, payload)
        assert kv.schedule_load("r1", tokens) == 2 * BLOCK
        assert kv.collect("r1") == payload
