"""Phase B7 — DeepSpeed-MII adapter lossy KV-tensor compression mode.

Exercises the shared ``kvcache_core.compress_store/compress_retrieve`` helper
through MII's put/get verbs, mirroring the SGLang/AIBrix/vLLM coverage.
"""
...
def test_compressed_put_get_round_trip():
    ...
    with MIIKVCache(tenant_id="mii-zc", model_id="mii-zc-demo",
...
def test_compressed_stored_smaller_than_raw():
    ...
    payload = _smooth_kv_bytes(len(key))
    with MIIKVCache(tenant_id="mii-zc", model_id="mii-zc-size",
                    bytes_per_token=BYTES_PER_TOKEN, compress=True) as kv:
        kv.put(key, payload)
...
def test_compress_requires_fp32_aligned():
    with pytest.raises(ValueError):
        MIIKVCache(tenant_id="t", model_id="m",
