"""Async load driver for the DeepSpeed-MII adapter.

Identical in shape to the SGLang / AIBrix / Dynamo / LMDeploy / TGI drivers
— a worker-pool wrapper around ``KVCacheConnector.fetch`` — reshaped to
DeepSpeed-MII's vocabulary. Per-adapter copy by convention (tiny shared
slice, independent release cadences); extraction into kvcache_core is
mechanical if needed.

The driver's async kick-off verb is ``prefetch(rid, key)`` — begin loading
the matched-prefix KV so a later ``pop`` returns instantly.
...
class AsyncLoadDriver:
    ...
    def __init__(self, connector: _ConnectorLike, *,
        ...
        self._executor = concurrent.futures.ThreadPoolExecutor(
            max_workers=workers,
            thread_name_prefix="kvcache-mii-load")
