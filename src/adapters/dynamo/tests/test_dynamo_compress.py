"""Phase B7 — Dynamo KVBM adapter lossy KV-tensor compression mode.

Exercises the shared ``kvcache_core.compress_store/compress_retrieve`` helper
through KVBM's offload/onboard verbs, mirroring the SGLang/AIBrix/vLLM coverage.
"""
...
def test_compressed_offload_onboard_round_trip():
    key = list(range(2 * CHUNK))
    payload = _smooth_kv_bytes(len(key))
    with DynamoKVBMConnector(tenant_id="dyn-zc", model_id="dyn-zc-demo",
                             bytes_per_token=BYTES_PER_TOKEN, compress=True) as kv:
        kv.offload(key, payload)
        got = kv.onboard(key)
...
def test_compressed_stored_smaller_than_raw():
    ...
    payload = _smooth_kv_bytes(len(key))
    with DynamoKVBMConnector(tenant_id="dyn-zc", model_id="dyn-zc-size",
                             bytes_per_token=BYTES_PER_TOKEN, compress=True) as kv:
        kv.offload(key, payload)
...
def test_compress_requires_fp32_aligned():
    with pytest.raises(ValueError):
        DynamoKVBMConnector(tenant_id="t", model_id="m",
