"""AsyncLoadDriver + AsyncAIBrixKVConnector tests.

Mirrors the SGLang async test shape from Phase Q-SG-1 — same fake-
connector approach for lifecycle/threading invariants (no
libkvcache.so), one real C-ABI round-trip at the bottom.
"""
...
from kvcache_aibrix import AsyncLoadDriver
...
class FakeConnector:
    """Mirror of the SGLang test's fake — same shape, same behaviour."""
...
def test_prefetch_miss_schedules_nothing():
    ...
    assert d.prefetch("req-1", [1, 2, 3]) == 0
    assert d.in_flight() == 0
    assert fc.fetch_calls == 0


def test_prefetch_hit_then_pop_returns_bytes():
    fc = FakeConnector()
    fc.store([1, 2, 3, 4], b"\x00" * (4 * BPT))
    d = AsyncLoadDriver(fc, bytes_per_token=BPT)
    matched = d.prefetch("req-1", [1, 2, 3, 4, 99])
    assert matched == 4
...
def test_fetch_is_actually_async_prefetch_returns_before_fetch_done():
    fc = FakeConnector()
    fc.store([7], b"x" * BPT)
    fc.fetch_gate.clear()
    d = AsyncLoadDriver(fc, bytes_per_token=BPT)
    t0 = time.monotonic()
    matched = d.prefetch("r", [7])
    elapsed = time.monotonic() - t0
    assert matched == 1
    assert elapsed < 0.5, f"prefetch blocked {elapsed:.3f}s"
...
def test_prefetch_replacing_in_flight_rid_releases_prior_handle():
    fc = FakeConnector()
    fc.store([1], b"a" * BPT)
    fc.store([2], b"b" * BPT)
    d = AsyncLoadDriver(fc, bytes_per_token=BPT)
...
def test_async_connector_prefetch_pop_round_trip_against_real_abi():
    ...
    with AsyncAIBrixKVConnector(tenant_id="aibrix-async-tenant",
                                 model_id="aibrix-async-demo",
                                 bytes_per_token=BYTES_PER_TOKEN,
                                 workers=2) as kv:
        kv.put(key, payload)
        matched = kv.prefetch("req-async-1", key)
