"""Async load driver for the TGI adapter.

Identical in shape to the SGLang / AIBrix / Dynamo / LMDeploy drivers — a
worker-pool wrapper around ``KVCacheConnector.fetch`` — reshaped to TGI's
vocabulary. Per-adapter copy by convention (tiny shared slice, independent
release cadences); extraction into kvcache_core is mechanical if needed.
...
class AsyncLoadDriver:
    ...
    def __init__(self, connector: _ConnectorLike, *,
        ...
        self._executor = concurrent.futures.ThreadPoolExecutor(
            max_workers=workers,
            thread_name_prefix="kvcache-tgi-load")
