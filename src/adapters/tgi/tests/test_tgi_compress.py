"""Phase B7 — TGI prefix-cache adapter lossy KV-tensor compression mode.

Exercises the shared ``kvcache_core.compress_store/compress_retrieve`` helper
through TGI's insert/load verbs, mirroring the SGLang/AIBrix/vLLM coverage.
"""
...
def test_compressed_insert_load_round_trip():
    ids = list(range(2 * CHUNK))
    payload = _smooth_kv_bytes(len(ids))
    with TGIPrefixCache(tenant_id="tgi-zc", model_id="tgi-zc-demo",
                        bytes_per_token=BYTES_PER_TOKEN, compress=True) as kv:
        kv.insert(ids, payload)
        got = kv.load(ids)
...
def test_compressed_stored_smaller_than_raw():
    ...
    payload = _smooth_kv_bytes(len(ids))
    with TGIPrefixCache(tenant_id="tgi-zc", model_id="tgi-zc-size",
                        bytes_per_token=BYTES_PER_TOKEN, compress=True) as kv:
        kv.insert(ids, payload)
        hit = kv._cx.lookup(ids)
...
def test_compress_requires_fp32_aligned():
    with pytest.raises(ValueError):
        TGIPrefixCache(tenant_id="t", model_id="m",
