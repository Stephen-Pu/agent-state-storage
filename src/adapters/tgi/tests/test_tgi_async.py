"""AsyncLoadDriver + AsyncTGIPrefixCache tests (fake connector for
...
from kvcache_tgi import AsyncLoadDriver
...
def test_async_prefill_take_round_trip():
    from kvcache_tgi import AsyncTGIPrefixCache
    ids = list(range(2 * BLOCK))
    payload = bytes(((i * 23) & 0xFF
                      for i in range(len(ids) * BYTES_PER_TOKEN)))
    with AsyncTGIPrefixCache(tenant_id="tgi-async", model_id="tgi-async-demo",
                             bytes_per_token=BYTES_PER_TOKEN, workers=2) as pc:
        pc.insert(ids, payload)
        assert pc.prefill("r1", ids) == 2 * BLOCK
        assert pc.take("r1") == payload
